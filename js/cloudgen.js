...
function showCloudContactForm(selectedPackage) {
  ...
  // Name field
  const { input: nameInput, helpText: nameHelp } = createField('Ime', 'text', 'Ime', 'Vaše ime');

  // Phone field
  const { input: phoneInput, helpText: phoneHelp } = createField('Telefon', 'tel', 'Telefon', '+38106XXXXXXX');
  ...
  submitBtn.innerText = 'Pošaljite zahtev';
  submitBtn.disabled = true;
  form.appendChild(submitBtn);

  // Regex validations
  const nameRegex = /^[A-ZČĆŽŠĐ][a-zčćžšđ]{1,29}$/;
  const phoneRegex = /^\+38106\d{7,8}$/;
  ...
  function checkFormValidity() {
    ...
    if (nameValid) {
      ...
      nameHelp.innerHTML = "Ime mora da počne velikim slovom.";
    }
    ...
    if (phoneValid) {
      ...
      phoneHelp.innerHTML = "Telefon mora da počinje sa +38106 i ima 7-8 cifara.";
    }
    ...
    if (emailValid) {
      ...
      emailHelp.innerHTML = "Email se kuca kao 'email@primer.com'";
    }
